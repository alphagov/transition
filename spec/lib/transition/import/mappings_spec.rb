...
describe Transition::Import::Mappings do
  def create_test_sites
    ...
    @decc_site = create :site, abbr: 'decc' do |site|
      site.hosts << create(:host, hostname: 'www.decc.gov.uk')
    end
  end

  describe '.from_redirector_csv_file!' do
    context 'a single import', testing_before_all: true do
      before :all do
        create_test_sites
    ...
    context 'an import with updates', testing_before_all: true do
      before :all do
        create_test_sites
        Transition::Import::Mappings.from_redirector_csv_file!('spec/fixtures/mappings/ago_abridged.csv')
        Transition::Import::Mappings.from_redirector_csv_file!('spec/fixtures/mappings/updated/ago_updated.csv')
      end
      ...
      it 'has updated the old mapping with a new_url' do
        @ago_site.mappings.where(path: '/_layouts/feed.aspx').first.new_url.should == 'https://www.gov.uk/updated-url'
      end
    end
  end
